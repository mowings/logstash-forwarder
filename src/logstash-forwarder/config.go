...
func MergeConfig(to *Config, from Config) (err error) {
	...
	if from.Network.TLSHost != "" {
		if to.Network.TLSHost != "" {
			return fmt.Errorf("TLSHost  already defined as '%s' in previous config file", to.Network.TLSHost)
		}
		to.Network.TLSHost = from.Network.TLSHost
	}
